...
@property (nonatomic,weak) IBOutlet UIImageView * centerImgview;//中心播放按钮
...
-(void)setVideoLayerGravity:(PlayerLayerGravity)videoLayerGravity
{
    _videoLayerGravity = videoLayerGravity;
    // AVLayerVideoGravityResize,           // 非均匀模式。两个维度完全填充至整个视图区域
    // AVLayerVideoGravityResizeAspect,     // 等比例填充，直到一个维度到达区域边界
    // AVLayerVideoGravityResizeAspectFill  // 等比例填充，直到填充满整个视图区域，其中一个维度的部分区域会被裁剪
    switch (videoLayerGravity) {
        case PlayerLayerGravityResize:
            _playerLayer.videoGravity = AVLayerVideoGravityResize;
            break;
        case PlayerLayerGravityResizeAspect:
            _playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case PlayerLayerGravityResizeAspectFill:
...
- (void)setlandscapeLayout{
    ...
    [self landscapeHide];
    [_delegate startlandscapeLayout];
...
- (void)play{
    ...
    [_player play];
    self.centerImgview.hidden=_isPlaying;
...
- (IBAction)startStopGestureRecognizers:(UITapGestureRecognizer *)sender {
    if (self.isPlaying) {
        [self pause];
    }else{
        [self play];
