...
#import "SuperPhotoBrowser.h"
#import "UIImageView+WebCache.h"
#import "MBProgressHUD.h"
#import "PhotoView.h"

@interface SuperPhotoBrowser ()<UIScrollViewDelegate,PhotoViewDelegate>
{
    CGFloat lastScale;
    MBProgressHUD *HUD;
...
@implementation SuperPhotoBrowser
...
//    [[SDImageCache sharedImageCache] clearMemory];
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 64, screen_width, screen_height)];
...
-(void)addLabels{
    self.sliderLabel = [[UILabel alloc] initWithFrame:CGRectMake(screen_width/2-20, 64, 60, 30)];
...
-(void)addNameLabels{
    self.nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, screen_height/2+180, screen_width-40, 40)];
...
-(void)addTFview
{
    self.TFview = [[UITextView alloc] initWithFrame:CGRectMake(20, screen_height/2+180+40, screen_width-40, screen_height-(_nameLabel.frame.origin.y+50))];
    self.TFview.backgroundColor = [UIColor blackColor];
    self.TFview.userInteractionEnabled=YES;
    self.TFview.editable=NO;
    self.TFview.textColor = [UIColor whiteColor];
    self.TFview.font=[UIFont systemFontOfSize:15];
    if(self.imageDetailsArray.count>0){
        
        self.TFview.text =self.imageDetailsArray[self.currentIndex];
    }else{
        self.TFview.text =@"";
    }
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if(i>=1){
        ...
        self.TFview.text=self.imageDetailsArray[i-1];
    }
}
