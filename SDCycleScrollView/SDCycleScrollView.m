...
@property (nonatomic,assign)CGPoint jlpoint;

@property (nonatomic,assign)NSInteger  bubianIndex;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self initializationWithautoscroll:YES infiniteloop:YES];
        [self setupMainView];
    }
    return self;
}
...
-(instancetype)initWithFrame:(CGRect)frame autoscroll:(BOOL)isautoscroll infiniteloop:(BOOL)isinfiniteloop
{
    if (self=[super initWithFrame:frame]) {
        [self initializationWithautoscroll:isautoscroll infiniteloop:isinfiniteloop];
        [self setupMainView];
    }
...
- (void)initializationWithautoscroll:(BOOL)isautoscroll infiniteloop:(BOOL)isinfiniteloop
{
...
-(void)setImageArrGroup:(NSArray *)imageArrGroup
{
    _imageArrGroup=imageArrGroup;
    self.imageURLsGroup =[[NSMutableArray alloc]initWithArray:imageArrGroup];
}
#pragma mark - actions
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.imageArrGroup[0] isKindOfClass:[UIImage class]]) {
        cell.imageView.image=self.imageArrGroup[itemIndex];

    }else{
    NSURL * urlstr=self.imageURLsGroup[itemIndex];
    if ([urlstr.absoluteString containsString:@"."]) {
        NSURL *url = self.imageURLsGroup[itemIndex];
        
        [cell.imageView sd_setImageWithURL:url placeholderImage:self.placeholderImage];
    }else{
        cell.imageView.image=[UIImage imageNamed:urlstr.absoluteString];
    }
    }
...
{
 UIImageView * imageviw= [self viewWithTag:121];
    if (imageviw) {
      
        return;
    }
    _bubianIndex=_curIndex;
    imageviw=[[UIImageView alloc]initWithFrame:self.bounds];
    [imageviw sd_setImageWithURL:[NSURL URLWithString:_imageURLStringsGroup[_bubianIndex]]];
   // imageviw.contentMode=self.bannerImageViewContentMode;
    imageviw.tag=121;
...
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(nullable UIView *)view atScale:(CGFloat)scale
{
    ...
    [_mainView setZoomScale:1 animated:NO];
    _mainView.contentOffset=CGPointMake(ww*_bubianIndex, 0);
